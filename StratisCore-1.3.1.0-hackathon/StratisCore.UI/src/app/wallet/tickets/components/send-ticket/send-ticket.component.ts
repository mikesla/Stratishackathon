...
@Mixin([Disposable])
export class SendTicketComponent implements OnInit {
  ...
  ngOnInit() {
    this.title = 'Send ticket ' + this.ticket.ticker;
  ...
  onSubmit() {
    ...
    this.title = 'Sending ticket ' + this.ticket.ticker + '...';
    ...
    this.smartContractsService.PostCall(result)
      ...
      .then(callResponse => {
        this.loading = false;
        this.activeModal.close({ request: result, callResponse, amount: this.ticketAmount.value, recipientAddress: this.recipientAddress.value });
      },
        error => {
          this.loading = false;
  ...
  private registerControls() {
    ...
    this.ticketAmount = new FormControl(0, [Validators.required, Validators.min(0), Validators.max(this.ticket.balance)]);
