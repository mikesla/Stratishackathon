...
export class AddTicketComponent implements OnInit, OnDestroy, Disposable {
  ...
  onSubmit() {
    ...
    if (addedTickets) {
      this.showApiError(`Ticket ${ticker} is already added`);

      return;
    }

    // Sender doesn't matter here, just reuse an easily available address
    const tickerCall = new LocalCallRequest(address, address, 'TotalSupply');
    ...
    this.ticketService
      ...
      .subscribe(localExecutionResult => {
        ...
        if (!methodCallResult) {
          this.showApiError(`Address is not a valid ticket contract.`);
          return;
        }
